...
import numpy as np
from bayes_dip.utils import tv_loss_3d_mean, batch_tv_grad_3d_mean
...
def params_and_image_rsamples_and_jac_and_manual_weight_and_image_samples():
    ...
    scale_params = nn.Parameter((torch.rand(dp) + 1.)*0.5)
    params = (scale_params,)
...
def test_first_derivative(params_and_jac_and_image_and_weight_samples):
    ...
    tv_x_samples = batch_tv_grad_3d_mean(x_samples)
    ...
    loss = tv_loss_3d_mean(x_samples)
...
def test_first_derivative_dist_rsamples(params_and_image_rsamples_and_jac_and_manual_weight_and_image_samples):
    ...
    manual_loss = tv_loss_3d_mean(manual_x_samples)

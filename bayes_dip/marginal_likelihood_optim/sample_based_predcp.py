...
def set_sample_based_predcp_grads(
        observation_cov: ObservationCov, 
        params_list_under_GPpriors: Sequence, 
        num_samples: int = 100,
        scale: float = 1.):
    ...
    second_derivative_grads = [autograd.grad(log_det,
        log_params, allow_unused=True, retain_graph=True)[0] for log_det, log_params in zip(log_dets, params_list_under_GPpriors)]
    
    with torch.no_grad():
        grads = compute_log_hyperparams_grads(params_list_under_GPpriors, first_derivative_grads, second_derivative_grads, scale)

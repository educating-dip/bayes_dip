        ...
        with tqdm(range(optim_kwargs['iterations']), desc='sample_based_marginal_likelihood_optim') as pbar:
            for i in pbar:
                ...
                eff_dim = estimate_effective_dimension(
                    ...
                    noise_variance=observation_cov.log_noise_variance.exp().detach()).clamp(
                        min=1, max=np.prod(observation_cov.trafo.obs_shape)-1)
                ...
                writer.add_image('linearized_model_recon', normalize(linearized_recon[0]), i)

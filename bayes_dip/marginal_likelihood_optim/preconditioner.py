...
class LowRankPreC(BasePreC):
    ...
    def sample(self, 
        ...
        empty_std = torch.empty(
            self.pre_con_obj.shape[0], num_samples, 
            device=self.pre_con_obj.device
            )
        empty_low_rank = torch.empty(
            self.pre_con_obj.low_rank_rank_dim, num_samples, 
            device=self.pre_con_obj.device
            )
        samples = empty_std.normal_() * torch.exp(self.pre_con_obj.log_noise_variance).pow(0.5) + (
            self.pre_con_obj.U * self.pre_con_obj.L.pow(0.5) ) @ empty_low_rank.normal_() 
        return samples 

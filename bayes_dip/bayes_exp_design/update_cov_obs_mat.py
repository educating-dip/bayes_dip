...
def update_cov_obs_mat_no_noise(
    ...
    for i in range(0, row_numel, batch_size):
        if ray_trafo_top_k_obj.matrix.is_sparse:
            v = torch.stack([ray_trafo_top_k_obj.matrix[j] for j in range(i, min(i+batch_size, row_numel))]).to_dense()
        else:
        ...
        if eff_batch_size < batch_size:
            v = v[:eff_batch_size]
        top_k_cov_obs_diag.append((ray_trafo_top_k_obj.matrix @ v.T).T)
        top_k_cov_obs_off_diag.append((ray_trafo_obj.matrix @ v.T).T)

...
from bayes_dip.utils.plot_utils import (
        DEFAULT_COLORS, configure_matplotlib, plot_hist, plot_image, plot_qq, add_inner_rect, add_metrics,
        add_log_lik)
...
recon = get_recon(dip_mll_optim_run, **kwargs)
...
try:
    ...
    recon_mcdo = compute_mcdo_reconstruction(
            run_mcdo, sample_idx=0, device='cpu').squeeze(1).squeeze(0)
abs_diff_mcdo = torch.abs(recon_mcdo - ground_truth)
stddev_mcdo = get_mcdo_stddev(run_mcdo, **kwargs, **stddev_kwargs)
...
slice_0, slice_1 = (
    ...
    else get_walnut_2d_inner_part_defined_by_patch_size(args.patch_size))
assert mask.sum() == (slice_0.stop - slice_0.start) * (slice_1.stop - slice_1.start)


print('computing bayes_dip log prob')
log_lik_predcp = restrict_sample_based_density_data_to_new_patch_idx_list(
        ...
        im_shape=ground_truth.shape)['log_prob']
print('computing mcdo log prob')
log_lik_mcdo = restrict_sample_based_density_data_to_new_patch_idx_list(
...
psnr = PSNR(
        recon[slice_0, slice_1].cpu().numpy(), ground_truth[slice_0, slice_1].cpu().numpy())
ssim = SSIM(
        recon[slice_0, slice_1].cpu().numpy(), ground_truth[slice_0, slice_1].cpu().numpy())
psnr_mcdo = PSNR(
        recon_mcdo[slice_0, slice_1].cpu().numpy(), ground_truth[slice_0, slice_1].cpu().numpy())
ssim_mcdo = SSIM(
        recon_mcdo[slice_0, slice_1].cpu().numpy(), ground_truth[slice_0, slice_1].cpu().numpy())


qq_err_map = normalized_error_for_qq_plot(
        recon[slice_0, slice_1],
        ground_truth[slice_0, slice_1],
        stddev_predcp[slice_0, slice_1])
qq_err_mcdo = normalized_error_for_qq_plot(
        recon_mcdo[slice_0, slice_1],
        ground_truth[slice_0, slice_1],
        stddev_mcdo[slice_0, slice_1])


print('getting pseudo 2d sinogram')
observation_2d = get_walnut_pseudo_2d_sinogram(cfg, walnut_data_path=walnut_data_path)
...
stddev_mcdo[torch.logical_not(mask)] = 0.
...
add_metrics(axs[0, 2], psnr, ssim)
plot_image(fig, axs[1, 2], recon_mcdo, vmin=0., insets=insets)
add_inner_rect(axs[1, 2], slice_0, slice_1)
add_metrics(axs[1, 2], psnr_mcdo, ssim_mcdo)
axs[0, 2].set_ylabel('Bayes DIP (TV-MAP)', fontsize=plt.rcParams['axes.titlesize'])
axs[1, 2].set_ylabel('DIP-MCDO', fontsize=plt.rcParams['axes.titlesize'])
# spacer
axs[0, 1].remove()
axs[1, 1].remove()
...
add_log_lik(axs[0, 5], log_lik_predcp)
...
osm_map, osr_map = scipy.stats.probplot(qq_err_map.flatten(), fit=False)
osm_mcdo, osr_mcdo = scipy.stats.probplot(qq_err_mcdo.flatten(), fit=False)
plot_qq(
        ax=ax_qq,
